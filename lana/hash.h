 ...
 * keyed on u32, look in intkeyedhash.h. The code is pretty much the
 * same, but the u32 code has more optimisations in it, notably the
 * comparisions are (obviously) much simpler.
 * 
 * You'll also notice this version is always HashKey/Value. The IntKeyedHash
 * can store anything.
 */
...
struct HashEnt {
    ...
    inline bool isUsed(){
        return k.type && k.type != Types::vtDeleted;
    }
...
public:
    ...
    HashEnt *look(Value *k,u64 hash){
        ...
        if(ent->isDeleted())
            freeslot = ent;
        else if(ent->hash == hash && ent->k.equalsForHashTable(k))
            return ent;
        else 
            freeslot = NULL;
        ...
        for(unsigned int perturb = hash;;perturb>>=PERTURB_SHIFT){
            ...
            if(!ent->isDeleted() && ent->k.equalsForHashTable(k))
                return ent;
